<template>
    <div class="p-3 rounded-xl bg-white flex flex-col gap-4">
        <!-- Title Pages -->
        <title-pages> Blog And News </title-pages>
        <!-- Endt Title Pages -->
        <!-- Mesasge -->
        <div
        ...
        <!-- End Message -->
        <div class="flex justify-between mb-10 hape:gap-2">
            <input
                type="search"
                name=""
                id=""
        ...
        >
            <!-- Card Staic Pages -->
            <div
                v-for="blogNews in this.data.data"
            ...
            >
                <div class="h-32 overflow-hidden">
                    <img
                        ...
                        :src="'/storage/img/blog-news/' + blogNews.image"
                        alt=""
                    />
                </div>
                <div
                    class="text-xs text-gray-700 p-1 text-center font-semibold rounded-md h-5"
                >
                    <span class="">{{ blogNews.title }}</span>
                </div>

                <div class="p-2 flex flex-col gap-2">
                    <button
                        @click="showData(blogNews.id)"
            ...
            >
                <div
                    class="mx-auto w-2/4 hape:w-full hape:mt-72 hape:relative bg-white drop-shadow-lg rounded-lg"
                >
                    <div class="p-5">
                        ...
                        <div v-if="updateMode" class="flex justify-center">
                            ...
                            <div v-else>
                                <img
                                    class="rounded-md max-w-sm mx-auto max-h-60"
                                    :src="
                                        '/storage/img/blog-news/' + this.image
                                    "
                                    alt=""
                                />
                            </div>
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (!this.updateMode) {
                router.post("/dashboard/blog-news/create-data", this.form, {
                    preserveScroll: true,
                    onSuccess: () => {
                        this.closedModal();
                    },
                ...
                router.post(
                    "/dashboard/blog-news/update-data/" + this.singelData[0].id,
                    this.form,
                    {
                        preserveScroll: true,
                        onSuccess: () => {
        ...
        deleteButton(id) {
            router.delete("/dashboard/blog-news/delete-data/" + id, {
                preserveScroll: true,
                onSuccess: () => {
                    this.closedModal();
                },
