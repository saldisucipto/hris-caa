    ...
    <div class="p-3 rounded-xl bg-white flex flex-col gap-4">
        ...
        <div class="m-3 bg-slate-100 rounded-lg">
            <div class="p-3 flex flex-col">
                <div>
                    <form
                        @submit.prevent="formCreatePeringatan"
                    ...
                    >
                        ...
                        <div class="flex justify-center gap-3">
                            <div class="flex flex-1 flex-col gap-2">
                                <label class="text-gray-700 font-semibold"
                                    >Jenis Peringatan</label
                                >
                                <select
                                    class="drop-shadow-sm border py-2 px-3 rounded-md focus:outline-none text-sm"
                                    v-model="peringatanForm.id_jenis_peringatan"
                                    name=""
                                    id=""
                                >
                                    <option
                                        v-for="jenisPeringatan in this
                                            .jenis_peringatan"
                                        :value="jenisPeringatan.id"
                                    >
                                        {{ jenisPeringatan.jenis_peringatan }}
                                    </option>
                                </select>
                            ...
                            <div class="flex flex-1 flex-col gap-2">
                                ...
                                <div
                                    class="text-xs px-1 text-red-600"
                                    v-if="errors.tanggal_peringatan"
                                >
                                    {{ errors.tanggal_peringatan }}
                                </div>
                            </div>
                        </div>
                        <div class="flex justify-center gap-3">
                            <div class="flex flex-1 flex-col gap-2">
                                <label class="text-gray-700 font-semibold"
                                    >Deskripsi Peringatan / Notes</label
                                >
                                <textarea
                                    class="drop-shadow-sm border py-2 px-3 rounded-md focus:outline-none text-sm"
                                    v-model="
                                        peringatanForm.deskripsi_peringatan
                                    "
                                    name=""
                                    id=""
                                    cols="5"
...
export default {
    ...
    setup() {
        ...
        const peringatanForm = useForm({
            id_jenis_peringatan: 1,
            id_employee: null,
            tanggal_peringatan: null,
            deskripsi_peringatan: "Keterangan Peringatan",
        });
        return { form, peringatanForm };
    },

    methods: {
        formCreatePeringatan() {
            router.post(
                "/hris/karyawan/peringatan/create",
                this.peringatanForm,
                {
                    preserveScroll: true,
                    onSuccess: () => {
        ...
        selectEmployee(id, namaKaryawan, jabatanKaryawan) {
            this.modal = false;
            this.peringatanForm.id_employee = id;
            this.nama_karyawan = namaKaryawan;
            this.jabatan_employee = jabatanKaryawan;
        },
        kalkulasiTanggal() {
            let tanggalMulaiCuti = new Date(
                this.peringatanForm.tanggal_peringatan
            );
            let tanggalAkhirCuti = new Date(
                this.peringatanForm.tanggal_akhir_cuti
            );
            ...
            this.peringatanForm.jumlah_cuti = days_difference + 1;
        },
    },
